...
For a substantial current example, inspect the current migrations and server fixtures under
`apps/server/src/persistence/Migrations/` before constructing inserts. Adapt the column set to the
target database instead of assuming copied SQL remains current.
