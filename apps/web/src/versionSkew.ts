 ...
 *
 * Prerelease and build suffixes are ignored. A server ahead of the client does
 * not need an update. Versions that do not parse as semver fall back to plain
 * string inequality.
 */
export function resolveVersionMismatch(
  ...
  return {
    ...
    hint: "Version mismatch. Restart the T3 Code server from its source checkout so both it and this client run the same version.",
  };
}
