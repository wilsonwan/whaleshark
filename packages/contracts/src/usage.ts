 ...
 *
 * v5 added the `grok` bucket, which this build no longer reports. v4
 * Claude buckets remain valid, so mixed-version environments keep those
 * totals instead of treating every older server as stale.
 */
export const USAGE_MERGE_COMPATIBLE_SINCE = 4 as const;

export const UsageProviderKind = Schema.Literals(["claude"]);
 ...
 * is a *subset* of `outputTokens` (Anthropic
