...
describe("EnvironmentSupervisor", () => {
  ...
  it.effect("keeps the latest failure visible throughout the next connection attempt", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
        prepare: (attempt) =>
          attempt === 1 ? Effect.fail(transient("SSH connection timed out.")) : Effect.never,
      });
      const supervisor = yield* EnvironmentSupervisor.make(TARGET_ENTRY, {
        initiallyDesired: true,
      }).pipe(Effect.provide(harness.dependencies));
  ...
  it.effect("reports a stalled connection setup and clears it after recovery", () =>
    Effect.gen(function* () {
      const harness = yield* makeHarness({
      ...
      yield* TestClock.adjust("3 seconds");
      const recovered = yield* awaitState(supervisor.state, (state) => state.phase === "connected");
