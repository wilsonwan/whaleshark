...
Reverting files covers the selected project's workspace only: files the agent
wrote elsewhere in the same repository, or outside it, stay on disk. The selected
prompt and its attachments return to the composer for editing and
